...
def geocode_address(address: str):
    ...
    try:
        return locator.geocode(address, timeout=10)
    except:
        return None
...
if mode == "select":
    ...
    for _, row in customers.iterrows():
        loc = geocode_address(row["Address"])
        if loc:
            folium.CircleMarker(
    ...
    if click:
        ...
        for _, row in customers.iterrows():
            loc = geocode_address(row["Address"])
            if loc:
                d = (loc.latitude - click["lat"])**2 + (loc.longitude - click["lng"])**2
                if d < best_dist:
                    best_dist, best_id = d, row["ID"]
        if best_id and best_dist < 0.0005:
